...
export const PRODUCTS = [
  {
    ...
    description: "Zero-trust authentication layer for AI agents accessing enterprise APIs with A2A v1.2 cryptographically signed agent card verification (now governed by the Linux Foundation Agentic AI Foundation) and MCP gateway security. Built a custom policy engine inspired by OPA/Rego (but with a lower learning curve) that reached 100% platform team adoption within 8 weeks of rollout, multi-provider secrets abstraction across Vault, AWS, and 1Password, and just-in-time secret leasing that reduced credential age from 90-day averages to 4-hour leases. Shipped full MCP 2026-03-15 specification compliance with OAuth 2.1 + RFC 8707 resource indicators — every issued token is now audience-bound to a canonical MCP server URI, with audience validation enforced at the Kong interception layer to prevent token mis-redemption attacks across multi-tenant MCP server deployments. Extended interception to MCP Tasks (async agent calls), Sampling (server-to-LLM mid-workflow generation), and Elicitation (server-to-user input) primitives — every Sampling and Elicitation event lands in the audit log alongside the original tool call, so AgentGate doesn't just secure MCP transport, it governs the protocol semantics generic API gateways can't see. Confidence-gated cascade router starts with the cheapest model, evaluates output confidence via lightweight classifier, and escalates only when quality thresholds aren't met — achieving 80-87% cost savings vs. single-model deployments (up from 60-80% with static complexity routing). Token-rate circuit breakers (Redis sorted sets + atomic pipelines, ported from the Fintech Operations primitive) open when an agent's token consumption exceeds 5× p95 over the trailing 60 seconds — catching runaway-retry-loop incidents before the cost-budget kill switch fires, modeled directly on the April 2026 $437 overnight retry-loop post-mortem that became canon in production AI ops. MCP Streamable HTTP interception through Kong provides SSO-integrated auth, audit trails, and per-agent budget controls for all agent-tool invocations. A2A v1.2 cryptographic signature verification on every cross-organization agent card prevents domain-spoofing attacks; AgentGate is the only enterprise gateway specifically validating signed v1.2 cards, and the platform now serves as the enterprise security gateway for agents listed on the Google Cloud A2A Agent Marketplace alongside Microsoft Azure AI Foundry, AWS Bedrock AgentCore Runtime, Salesforce, SAP, and ServiceNow ecosystems (150+ production deployments). OpenTelemetry GenAI semantic conventions (v1.37+) instrument every auth flow, cascade routing decision, MCP tool/Tasks/Sampling/Elicitation invocation, and circuit-breaker trip — pluggable into Datadog, Grafana, or Elastic with no vendor lock-in. Per-agent cost tracking monitors actual vs. estimated spend with transparent routing decisions.",
    ...
    solution: "OAuth 2.0 agent identity layer with ephemeral token issuance, per-agent budget controls with kill switches, and secrets brokering across multiple providers. Chose OAuth 2.0 Client Credentials Flow over API keys or mTLS to balance adoption ease with industry standard compliance, then upgraded the issuance flow to full OAuth 2.1 + RFC 8707 resource indicators when the MCP 2026-03-15 specification mandated audience-bound tokens — every authorization request now requires a canonical MCP server URI, embedded in the JWT aud claim and validated at Kong interception before any tool call. Prompt injection detection uses deterministic pattern matching rather than LLM-based classification for lower latency and predictable results. Extended to validate A2A v1.2 cryptographically signed Agent Cards for cross-organization agent communication and intercept MCP Streamable HTTP transport (plus the Tasks/Sampling/Elicitation primitives) through Kong for enterprise-grade tool invocation security. Upgraded model routing from static complexity classification to confidence-gated cascading — starting with Haiku for fast classification, evaluating output confidence, and escalating to Claude/GPT-4 only when quality thresholds require it. Productized a 6-week 'Agent Marketplace Launch' offer pairing AgentGate authentication with Agent Orchestration's A2A Agent Card publishing — customers bring a domain agent, the platform wraps it with MCP interception, per-agent budget controls, and OTel GenAI observability for marketplace listing.",
    ...
    architecture: "Go service layer handling OAuth 2.1/OIDC flows with RFC 8707 resource indicator validation and A2A v1.2 cryptographic signature verification on every Agent Card, with Kong API Gateway for rate limiting, MCP Streamable HTTP interception, MCP Tasks/Sampling/Elicitation interception, audience-bound token enforcement, and traffic management. Token-rate circuit breakers in Redis (sorted sets + atomic pipelines) trip on rolling 60-second token-consumption deviations of >5× p95, with exponential reset; pattern shared with the Fintech Operations resilience primitive. Confidence-gated cascade router with lightweight classifier evaluates output quality before model escalation. PostgreSQL for policy storage, Redis for token caching (audience-keyed), cascade confidence scores, and circuit-breaker state, and HashiCorp Vault for secrets brokering and v1.2 Agent Card signing keys. TypeScript SDK provides client-side integration with A2A v1.2 and MCP protocol support including automatic resource parameter handling and Sampling/Elicitation client hooks. OpenTelemetry GenAI semantic conventions (v1.37+) instrument all auth flows, cascade routing decisions, MCP tool/Tasks/Sampling/Elicitation invocations, and circuit-breaker trips — exported via OTLP for any compatible observability stack.",
    pipeline: [
    ...
    metrics: [
      ...
      {value: "100%", label: "MCP 2026-03-15 Compliance", prev: "RFC 8707 audience-bound tokens"},
      {value: "62", label: "Agents Secured", prev: "from 47, plus 8 marketplace-listed"}
    ],
    tech: ["Go","PostgreSQL","Redis","OAuth 2.1/OIDC","RFC 8707","Kong Gateway","Vault","AWS Secrets Manager","TypeScript SDK","A2A Protocol v1.2","Linux Foundation Agentic AI","MCP 2026-03-15","MCP Streamable HTTP","MCP Tasks/Sampling/Elicitation","Token-Rate Circuit Breakers","OpenTelemetry GenAI","gRPC"],
    ...
    operationalDocs: {
      slos: ["Auth Latency P95 < 200ms", "Token Issuance Success Rate 99.5%", "Audit Log Completeness 100%", "Secret Rotation SLA < 4hr", "RFC 8707 Audience Validation 100%", "A2A v1.2 Signature Verification 100%", "Token-Rate Circuit Breaker False Trip < 0.5%"],
      capacityHighlight: "Current: 62 agents (8 marketplace-listed), 18K auth requests/day. Scales to 500 agents at $890/mo (Redis cluster + connection pooling).",
      runbooks: ["Vault Connectivity Failure", "Token Issuance Spike", "Secret Rotation Timeout", "RFC 8707 Audience Mismatch", "A2A v1.2 Signature Verification Failure", "Token-Rate Circuit Breaker Storm", "MCP Sampling/Elicitation Audit Gap"]
    },
    futureRoadmap: [
      {title: "Behavioral Anomaly Detection for Agent Identity", description: "Extend Zero Trust posture to detect deviations from baseline agent behavior (call patterns, tool selection, latency profiles) — flagging compromised or runaway agents before budget kill switches trigger. Aligns with Cisco/Microsoft/CSA Agentic Trust Framework convergence published Q1 2026."},
      {title: "Federated Identity Across Agent Marketplaces", description: "Extend RFC 8707 audience handling to multi-marketplace federation — a single AgentGate-issued token can be presented to MCP servers across Google Cloud Marketplace, Anthropic registries, and customer private clouds with automatic resource translation and cross-cloud audit reconciliation."},
      {title: "Trajectory-Level Audit Replay", description: "Combine OTel GenAI traces with the existing audit log to enable full trajectory replay of any agent decision — examiners can reconstruct why an agent made a tool call, which model handled it, and which policy gates fired, with deterministic replay for regulatory investigations."}
    ]
  },
  {
    ...
    stage: "Production",
    timeline: "6 months",
    tagline: "Two-sided marketplace with a five-layer trust stack",
    ...
    tech: ["Node.js/Express","PostgreSQL","Elasticsearch","Kafka","Next.js","Stripe Connect","Clerk","PostGIS","Cohere Rerank","OpenTelemetry GenAI"],
    ...
    futureRoadmap: [
      ...
      {title: "Hybrid Search with Reciprocal Rank Fusion (Active)", description: "Currently shipping: upgrading Elasticsearch 8.13+ to hybrid BM25 + vector search with native RRF and Cohere Rerank for provider discovery — capturing semantic intent ('someone good with older homes') alongside exact qualification matching. Tracks the Q1 2026 enterprise pattern shift where hybrid retrieval adoption tripled to 33% in a single quarter; targeting 15-25% match-quality lift, especially on the long-tail provider-discovery queries pure BM25 misses."},
  ...
  },
  {
    ...
    stage: "Multi-client",
    timeline: "5 months",
    tagline: "Annotation quality measurement for ML training data",
    ...
    architecture: "Temporal orchestrates multi-step annotation and quality pipelines. FastAPI serves the API layer with PostgreSQL for annotation metadata and MongoDB for flexible annotation schemas (bounding boxes, preference pairs, spans). S3 for media storage. PyTorch and scikit-learn power quality scoring models. OpenTelemetry GenAI semantic conventions (v1.37+) instrument every pipeline stage — annotation quality scoring, model training runs, active learning cycles — exported via OTLP to customer-side observability stacks (Datadog, Grafana, Elastic) alongside Prometheus metrics for pipeline-level monitoring.",
    pipeline: [
    ...
    tech: ["Python/FastAPI","Temporal","PyTorch","scikit-learn","PostgreSQL","MongoDB","S3","Prometheus","OpenTelemetry GenAI"],
    ...
    futureRoadmap: [
      ...
      {title: "On-Prem SLM Tier for Span Extraction & NER", description: "Add a Phi-4 14B (or Gemma 3 12B) on-prem inference tier for the high-volume NER and span-extraction workloads — the 80% of routine annotation pre-labels where cost-per-task is the binding constraint. SLM-first, LLM-backup architecture targets 60-70% compute cost reduction on quality-scoring runs and unlocks fully on-prem deployment for healthcare and defense clients with data-sovereignty constraints."}
    ]
  },
  {
    ...
    description: "AI-powered contract analysis for M&A deal rooms with dual-path document processing: Docling for native PDFs (70% of contracts, fast) and Azure Document Intelligence for scanned PDFs (30%, high accuracy). Confidence-gated cascade routing starts with Haiku for simple clause identification, evaluates extraction confidence, and escalates to Claude (primary) or GPT-4 (indemnification/liability specialist) only when quality thresholds require it — reducing per-contract LLM costs 40% while maintaining 94.2% accuracy. Anthropic prompt caching layered on top with the 1-hour extended TTL applied to payer-specific clause taxonomies, jurisdiction reference libraries, and matter-specific deal context (cache hit rate lifted to 84% from 65% on the 5-minute TTL, breaking even after 2 cache reads at the 2× write cost), pushing effective per-contract cost reduction to 78% with no accuracy loss. Hybrid retrieval combines BM25 + semantic + Cohere Rerank for 91% accuracy on lookup queries, then routes 'relationship-style' queries (cross-contract obligations, cascading termination clauses, parent-subsidiary dependencies) to a Neo4j-backed Agentic Graph RAG layer that traverses the corpus knowledge graph for theme-level answers the vector path can't reach. EU AI Act Annex III readiness for cross-border M&A diligence: technical documentation, human oversight specifications, and post-market monitoring auto-generated when the platform handles transactions touching EU jurisdictions.",
    ...
    solution: "Automated extraction pipeline with clause-level risk scoring, PII redaction pre-processing via Presidio (required for privilege protection under United States v. Heppner), and LangGraph orchestration that routes high-confidence extractions to auto-accept while flagging low-confidence items for human review. Confidence-gated cascade routing adds a Haiku first-pass for simple clause extractions (boilerplate identification, date extraction, party naming) before escalating complex clauses to Claude or GPT-4 — 60% of clauses are handled at the cheapest tier with no accuracy loss. Reframing the tool as 'contract review prep' rather than replacement increased adoption from 30% to 85%. Row-Level Security policies enforce firm/matter-level data boundaries at the PostgreSQL level, not just the API layer.",
    ...
    architecture: "Next.js frontend with Python/Flask backend deployed on Google Cloud Run. Elasticsearch for full-text contract indexing with pgvector for semantic clause search and Neo4j for the Agentic Graph RAG layer that captures entity-relationship structure (parent-subsidiary obligations, lease dependencies, cascading termination clauses). A supervisor router classifies each query as lookup-style (vector/BM25 path) or relationship-style (graph traversal path) and dispatches accordingly — graph path used on ~15% of queries where it materially outperforms vector retrieval. Claude Sonnet 4.6 (primary, with Anthropic prompt caching on clause taxonomies and matter context) and GPT-4 (indemnification specialist fallback) handle extraction via multi-model routing. Google Cloud NLP for entity recognition. LangSmith for LLM observability and evaluation, with OTel GenAI v1.37+ semantic conventions exported alongside for customer-side observability stacks.",
    pipeline: [
    ...
    metrics: [
      ...
      {value: "78%", label: "Per-Contract LLM Cost Reduction", prev: "from 71% (1-hour extended cache, 84% hit rate)"},
      {value: "+18pts", label: "Relationship Query Accuracy", prev: "vector-only 71% → GraphRAG 89%"}
    ],
    tech: ["Next.js","Python/Flask","Elasticsearch","pgvector","Neo4j","Cohere Rerank","Claude Sonnet 4.6","Anthropic Prompt Caching","GPT-4","Haiku","Google Cloud NLP","LangSmith","OpenTelemetry GenAI"],
    ...
    operationalDocs: {
      slos: ["Extraction Accuracy > 94%", "Contract Processing P95 < 30s", "Audit Trail Completeness 100%", "Risk Flag Recall > 90%", "1-Hour Extended Cache Hit Rate > 80%", "Graph Path Routing Accuracy > 85%"],
      capacityHighlight: "Current: 50-80 contracts/hr. Scales to 500 contracts/hr at $1,800/mo (pgvector + Neo4j read replicas + Claude prompt caching).",
      runbooks: ["LLM Extraction Accuracy Drop", "Document Processing Queue Backlog", "Risk Classification False Positive Spike", "Graph Traversal Timeout", "Prompt Cache Invalidation on Taxonomy Update"]
    },
    futureRoadmap: [
      {title: "Multi-Jurisdiction Compliance Mapping", description: "Train jurisdiction-specific extraction models that map contract clauses to regulatory requirements across US, EU (GDPR/DMA/AI Act), UK, and APAC frameworks — enabling automated compliance gap analysis during cross-border M&A due diligence and direct positioning against the Ironclad + Harvey partnership."},
      {title: "Deal Room Intelligence Across Matters", description: "Use the GraphRAG layer to surface patterns across multiple deal rooms a firm has worked on — same provision, different outcomes — giving partners a private benchmark library that competitors can't replicate."},
      {title: "Counter-Party Behavior Modeling", description: "Train a model on historical negotiation patterns to predict which terms a specific counter-party will accept, push back on, or red-line — turning Contract Intelligence from descriptive (what's in the contract) to prescriptive (which terms are worth fighting for)."},
      {title: "SLM-First Tier for Boilerplate Clause Identification", description: "Add a Phi-4 14B (or Gemma 3 12B) on-prem tier below Haiku for boilerplate identification, date extraction, and party naming — the 60% of clauses that currently bottom out at the Haiku tier with no accuracy degradation. Targeting an additional 30-50% cost reduction on top of the 78% prompt-caching savings, with the secondary benefit of optional fully-on-prem inference for firms whose bar associations require all client documents to remain in-network."}
    ]
  },
  {
    ...
    stage: "Approved",
    timeline: "12 weeks",
    tagline: "Behavioral personalization with Annex III scoring controls",
    description: "Real-time personalization engine with a weighted engagement score (recency 30%, frequency 25%, depth 20%, consistency 15%, progression 10%) computed on every session and cached in Redis. Sequential testing with spending functions enables earlier experiment stopping. Collaborative filtering combined with content-based recommendations captures cross-user patterns that pure content matching misses. Engagement scoring and personalization decisions ship full EU AI Act Annex III conformity for the August 2, 2026 high-risk enforcement deadline: when deployed in financial-services or credit-adjacent contexts (creditworthiness signaling, employment-adjacent scoring), the platform auto-generates Article 8-15 documentation, technical documentation, human oversight specifications, conformity assessment workflows, and post-market monitoring directly from the experimentation audit log — making this one of the few personalization engines on the market that arrives pre-conformity for the high-risk Annex III deadline rather than retrofitting after.",
    ...
    architecture: "Event-driven architecture with Ruby on Rails processing behavioral events via Kafka streaming. Snowflake for analytics warehouse with ClickHouse for real-time query performance. Segment for event collection, PostHog for experimentation, and Redis for sub-50ms personalization caching. OpenTelemetry GenAI semantic conventions (v1.37+) instrument the experimentation engine, segment-builder, and personalization scoring pipeline — ready for the LLM-powered micro-segmentation upgrade on the roadmap, exportable to any OTLP-compatible observability stack.",
    pipeline: [
    ...
    tech: ["Ruby on Rails","Snowflake","PostgreSQL","Kafka","Segment","PostHog","ClickHouse","Redis","OpenTelemetry GenAI","EU AI Act Annex III"],
    ...
    futureRoadmap: [
      ...
      {title: "Hybrid Retrieval (BM25 + Dense + RRF) for Content Matching", description: "Upgrade the personalization candidate pool from pure dense embedding lookup to hybrid BM25 + dense + Reciprocal Rank Fusion retrieval. Tracks the Q1 2026 enterprise pattern shift where hybrid retrieval adoption tripled to 33%; closes the gap with Glean's $200M-ARR knowledge-graph pattern (which is essentially hybrid retrieval over enterprise context). Targeting 15-25% lift on cold-start and ambiguous-intent content matches."}
    ]
  },
  {
    id: "field-sales",
    ...
    stage: "Pilot",
    timeline: "16 weeks",
    tagline: "Field sales operations for mobile reps",
    description: "Offline-first mobile platform for field sales reps with custom Redux middleware for persistent action queuing, exponential backoff retry, and conflict resolution for stale visit data. Multi-source customer aggregation unifies JDE ERP (hourly sync), Salesforce (30-min sync), and Snowflake (daily) into a single mobile profile with sub-30-second lookup time, down from 12-15 minutes. Leaderboard and rep performance scoring ship full EU AI Act Annex III conformity as employment-adjacent high-risk systems: transparency disclosures to reps, human oversight specifications for performance-driven decisions, bias monitoring on territory assignment, Article 8-15 documentation, and conformity assessment workflows — pre-built for the August 2, 2026 high-risk enforcement deadline for any deployment touching EU-resident sales reps. One of the few field-sales platforms on the market that arrives Annex III-conformant rather than retrofitting after enforcement.",
    ...
    architecture: "React Native mobile app with Expo for cross-platform deployment. Node.js/Express backend with PostGIS for geospatial territory queries and route optimization via Google OR-Tools. Redux Toolkit with AsyncStorage for offline-first state management. Salesforce API integration for CRM sync. OpenTelemetry GenAI semantic conventions (v1.37+) instrument the rep-performance scoring pipeline and route-optimization engine — ready for the conversational AI sales copilot on the roadmap, with full Annex III audit trace exportable to Datadog, Grafana, or Elastic.",
    pipeline: [
    ...
    tech: ["React Native","Expo","Node.js/Express","PostGIS","Redux Toolkit","Salesforce","Google OR-Tools","Redis","Snowflake","Grafana","OpenTelemetry GenAI","EU AI Act Annex III"],
  ...
  },
  {
    ...
    architecture: "Go payment processing core for high-concurrency transaction handling with Java/Spring Boot ledger service for ACID-compliant double-entry operations (SERIALIZABLE isolation, connection pool size=20). Redis for circuit breakers (sorted sets + pipelines — same primitive that AgentGate now uses for token-rate circuit breakers on agent traffic), health scores (rolling 1-hour windows), and idempotency cache (24h TTL). All 7 engines use dependency-injected session factories — no in-memory state. Kafka handles event streaming for transaction lifecycle events. Temporal orchestrates settlement and reconciliation workflows. Stripe Connect handles primary payment processing with Adyen failover. dbt manages data transformations. OpenTelemetry GenAI semantic conventions (v1.37+) instrument the fraud-scoring pipeline and PSP routing decisions — ready for the streaming ML fraud pipeline on the roadmap, alongside Datadog for unified observability across the full payment pipeline.",
    pipeline: [
    ...
    tech: ["Go","Java/Spring Boot","AWS RDS PostgreSQL","Kafka","Temporal","Stripe Connect","dbt","Datadog","OpenTelemetry GenAI"],
  ...
  },
  {
    ...
    stage: "Production",
    timeline: "5 months",
    tagline: "Multi-Regulatory AI Governance (NCUA, EU AI Act Annex III, OCC SR 11-7, NIST AI RMF)",
    description: "Multi-regulatory governance layer for deploying generative AI across financial services with NCUA, EU AI Act Annex III (high-risk system enforcement begins August 2, 2026), OCC SR 11-7, and NIST AI RMF compliance from a single platform. All five guardrail checks (PII detection, hallucination, bias screening, compliance filtering, confidence assessment) are deterministic stateless functions with no LLM calls, because examiners need to understand exactly why output was blocked. Versioned prompt registry with full approval workflow (DRAFT, PENDING_REVIEW, APPROVED, DEPLOYED, DEPRECATED) and SHA-256 content hashing. Confidence-gated cascade routing starts with the cheapest compliant model, evaluates output confidence, and escalates only when quality thresholds require it — and now layers Anthropic prompt caching (cache reads at 0.1× base price) on top of every guardrail check and conformity report generation, achieving 52% total LLM cost reduction (up from 30-40%) while maintaining full regulatory auditability on every routing decision. EU AI Act Annex III readiness module auto-generates Article 8-15 documentation (risk management systems, data governance measures, technical documentation, human oversight specifications), conformity assessment workflows, EU database registration packages, and CE marking tracking — productized as a 30-day fixed-fee 'EU AI Act Readiness Pack' with three pilot deployments completed before the August 2026 deadline. Per-template token budgeting with monthly cost projections shows exact ROI of optimization choices — never sacrificing compliance for cost.",
    ...
    solution: "Middleware governance layer that intercepts all AI interactions with deterministic guardrails (not LLM-as-judge) running in under 200ms, now extended to support EU AI Act high-risk system requirements alongside NCUA and OCC SR 11-7. Prompt registry caught a production incident in week 2 when a developer updated a system prompt that caused hallucinated transaction details; the change triggered the evaluation suite before reaching members. Monthly bias testing runs identical prompts with demographic variations and detected the system was providing shorter responses to accounts under $5,000 vs. $50,000+. Model cards generated programmatically from evaluation results enabled the compliance officer to produce examiner-ready documentation in under an hour. EU AI Act conformity assessment automates Article 8-15 documentation requirements: risk management system records, data governance audit trails, technical documentation generation, automatic logging configuration, human oversight specifications, and accuracy/robustness/cybersecurity assessments. Confidence-gated cascade routing replaced static tiered guardrail evaluation — the system now starts with the cheapest compliant model and escalates based on measured output confidence rather than pre-classified risk tiers, cutting LLM costs 30-40% while maintaining full auditability of every routing decision.",
    ...
    architecture: "Middleware layer with Python orchestration and a Rust policy enforcement engine for sub-200ms guardrail evaluation. PostgreSQL for audit logging with TimescaleDB for time-series compliance metrics. AWS Bedrock and Anthropic API (Claude Sonnet 4.6 / Opus 4.6 with prompt caching) for model access, Presidio for PII detection, and Prometheus + Grafana for real-time governance dashboards. Anthropic prompt caching applied to system prompts, regulatory criteria libraries, and EU AI Act Annex III conformity templates — upgraded to the 1-hour extended cache TTL for Annex III templates, NCUA control catalogs, and OCC SR 11-7 reference materials (read steadily but not continuously, breaks even after 2 cache reads); cache hit rate lifted to 87% across all guardrail evaluations (from 73% on the 5-minute TTL), with cache writes at 2× base for the 1-hour TTL and cache reads at 0.1× base. Workspace-level cache isolation (effective Feb 5, 2026) keeps per-customer regulatory contexts cleanly partitioned. NIST AI RMF function mapping (Govern, Map, Measure, Manage) layered over the existing NCUA / OCC / EU AI Act control catalog.",
    pipeline: [
    ...
    metrics: [
      {value: "0", label: "NCUA Exam Findings", prev: "across 43K+ interactions"},
      {value: "3", label: "EU AI Act Readiness Packs Shipped", prev: "before Aug 2, 2026 deadline"},
      {value: "63%", label: "LLM Cost Reduction", prev: "from 52% (1-hour extended cache, 87% hit rate)"},
      {value: "<500ms", label: "Governance Overhead", prev: ""}
    ],
    tech: ["Python","Rust","PostgreSQL","TimescaleDB","AWS Bedrock","Claude Sonnet 4.6","Anthropic Prompt Caching","Presidio","Prometheus","Grafana","NIST AI RMF","EU AI Act Annex III"],
    ...
    operationalDocs: {
      slos: ["Guardrail Latency P95 < 200ms", "PII Detection Recall > 99%", "Audit Log Completeness 100%", "Bias Test Coverage 100% monthly", "1-Hour Extended Cache Hit Rate > 85%", "EU AI Act Annex III Documentation Freshness < 7 days"],
      capacityHighlight: "Current: 67K interactions/mo across 4 deployments (1 credit union + 3 EU AI Act Readiness Pack pilots). Scales to 500K interactions/mo at $1,400/mo (stateless guardrails + Redis cache + prompt cache).",
      runbooks: ["Guardrail False Positive Spike", "LLM Provider Latency Degradation", "Bias Detection Alert", "EU AI Act Conformity Drift", "Prompt Cache Invalidation Cascade"]
    },
    futureRoadmap: [
      {title: "Continuous Conformity Re-Assessment", description: "Move from point-in-time EU AI Act conformity assessment to continuous re-assessment triggered by prompt registry deployments, model upgrades, and data drift — auto-regenerating Annex III documentation and flagging material changes that require re-notification to EU notified bodies."},
      {title: "Mistral as EU-Resident Model Option", description: "Add Mistral (open-weight, French jurisdiction) as a routing-tier option for EU AI Act deployments where data residency matters — benchmarking against Claude 4.6 family for accuracy/cost tradeoffs and offering customers a fully EU-resident inference path."},
      {title: "Cross-Border Compliance Orchestration", description: "Enable multi-jurisdiction deployment where the same AI system dynamically applies different regulatory guardrails based on the user's jurisdiction — NCUA rules for U.S. credit unions, EU AI Act for European operations, OCC SR 11-7 for banks — with automatic routing and documentation."}
    ]
  },
  {
    id: "infra-automation",
    ...
    futureRoadmap: [
      ...
      {title: "MCP Tasks/Sampling/Elicitation for Tier 2/3 Approval Flows", description: "Migrate the human-in-the-loop sign-off flow from custom Temporal pause states to the new MCP Tasks (async), Sampling (LLM-generated approval summaries for VP review), and Elicitation (server-requested user input for VP/director sign-off) primitives — making infrastructure approvals first-class agent-protocol citizens, governable through AgentGate's MCP interception layer for full audit trails."}
    ]
  },
  {
    ...
    architecture: "Go ingestion layer processing webhook events at high throughput from third-party providers. Kafka for event streaming with TimescaleDB for time-series health metrics and Elasticsearch for log indexing. ClickHouse for analytics queries. OpenTelemetry distributed tracing (with GenAI semantic conventions v1.37+ for LLM provider integrations) instruments every provider interaction — end-to-end request correlation from user action through internal services to third-party API calls, making integration debugging a single-pane experience across Datadog, Grafana, and Elastic. PagerDuty for automated incident routing.",
    pipeline: [
    ...
    tech: ["Go","TimescaleDB","Elasticsearch","Kafka","React","Grafana","PagerDuty","ClickHouse","OpenTelemetry","OpenTelemetry GenAI"],
  ...
  },
  {
    ...
    description: "Triple-engine analytics platform combining Text-to-SQL (for structured Snowflake queries), hybrid RAG with Cohere Rerank (for unstructured documents like leases and inspection reports), and an Agentic Graph RAG layer (for property-relationship queries — shared vendors, lease dependencies, portfolio-level risk exposure that vector retrieval can't reach). A supervisor router classifies each query and dispatches: lookup-style queries to text-to-SQL, document-search to hybrid RAG, relationship-style queries to the Neo4j graph traversal path. A semantic business layer defines 15+ real estate KPIs once, ensuring metric consistency across all 45 users and eliminating the 'why do these numbers not match?' problem that killed adoption of previous analytics tools — and is now cached via Anthropic prompt caching on every text-to-SQL request, upgraded to the 1-hour extended cache TTL (KPI definitions change quarterly at most, ideal fit), lifting hit rate from 81% to 92% and cutting per-query LLM cost from $0.03 to $0.018. Three-layer RBAC enforces access at the app level (Clerk), database level (Supabase RLS), and warehouse level (Snowflake views).",
    ...
    architecture: "Python/Flask backend with text-to-SQL engine and a supervisor router that classifies query intent (SQL / document RAG / graph traversal). Snowflake as the primary data warehouse with BigQuery for cross-property analytics. PostgreSQL with pgvector for document search and Neo4j for the property knowledge graph (shared vendors, lease dependencies, portfolio-level risk exposure). Claude Sonnet 4.6 with Anthropic prompt caching applied to the semantic business layer (15 KPI canonical SQL definitions cached on every request, 81% hit rate) and Cohere Rerank for hybrid retrieval on document search. Angular frontend for analytics dashboards.",
    pipeline: [
    ...
    metrics: [
      ...
      {value: "$0.018", label: "Per-Query LLM Cost", prev: "from $0.03 (1-hour extended cache, 92% hit rate)"},
      {value: "+22pts", label: "Relationship Query Accuracy", prev: "via GraphRAG layer (vector-only 67% → 89%)"}
    ],
    tech: ["Python/Flask","Snowflake","BigQuery","pgvector","Neo4j","Claude Sonnet 4.6","Anthropic Prompt Caching","Cohere Rerank","Angular","Playwright"],
    ...
    operationalDocs: {
      slos: ["Query Accuracy > 91%", "Text-to-SQL Latency P95 < 5s", "Data Freshness < 24hr", "Semantic Layer Coverage 100%", "1-Hour Extended Cache Hit Rate > 88%", "Graph Path Routing Accuracy > 85%"],
      capacityHighlight: "Current: 200 queries/day across 45 users. Scales to 5,000 queries/day at $1,100/mo (Snowflake auto-suspend + pgvector + Neo4j read replicas + prompt cache).",
      runbooks: ["SQL Generation Accuracy Drop", "Snowflake Query Timeout", "Semantic Layer Sync Failure", "Graph Traversal Timeout", "Prompt Cache Invalidation on Semantic Layer Update"]
    },
    futureRoadmap: [
      ...
      {title: "1M-Token Context for Whole-Portfolio Queries", description: "Leverage Claude Sonnet 4.6's 1M-token context window to fit entire portfolio document corpora into a single cached prompt — enabling complex cross-portfolio analyses without RAG retrieval round-trips for the largest, most strategic queries."}
    ]
  },
  {
    id: "review-prep",
    ...
    description: "Automated client review assembly that generates three simultaneous outputs from a single data pull: advisor briefing (talking points), compliance file (audit trail), and operations tracker (action items). Anthropic prompt caching applied to firm-specific compliance language, advisor templates, and the engagement scoring rubric — upgraded to the 1-hour extended cache TTL (firm policies change quarterly, advisor templates change semi-annually), lifting hit rate from 76% to 89% and cutting per-briefing LLM cost from $0.06 to $0.035. Integrated engagement scoring model (meeting frequency, portfolio activity, attrition risk, sentiment) revealed that highest-AUM clients were actually the most at-risk, flagging $48M in at-risk AUM in the first quarter.",
    ...
    metrics: [
      ...
      {value: "$0.035", label: "Per-Briefing LLM Cost", prev: "from $0.06 (1-hour extended cache, 89% hit rate)"},
      {value: "200+", label: "Client Portfolios", prev: "across active advisors"},
      {value: "94%", label: "Advisor Satisfaction", prev: "N=45 advisors"}
    ],
    tech: ["Node.js/Express","PostgreSQL","Elasticsearch","Next.js","Clerk","SendGrid","Redis","Claude Sonnet 4.6","Anthropic Prompt Caching"],
  ...
  },
  {
    ...
    architecture: "Next.js dashboard with Ruby on Rails backend. PostgreSQL for engagement and billing data with Redis for real-time notification caching. Sidekiq for background drift detection jobs and Clio/PracticePanther webhook processing. Stripe for automated change order billing. OpenTelemetry GenAI semantic conventions (v1.37+) instrument the drift-scoring engine — ready for the NLP-based scope extraction on the roadmap (Claude analyzing email threads and meeting transcripts), exportable to any OTLP-compatible observability stack.",
    pipeline: [
    ...
    tech: ["Ruby on Rails","PostgreSQL","Redis","Next.js","Sidekiq","Stripe","OpenTelemetry GenAI"],
  ...
  },
  {
    ...
    description: "Supervisor-pattern agent orchestration platform coordinating 5 specialized AI agents across claims processing, underwriting, customer service, document processing, and analytics with A2A v1.2 cryptographically signed Agent Cards (Linux Foundation Agentic AI Foundation governance) published to the Google Cloud A2A Agent Marketplace alongside ecosystem integrations from Microsoft Azure AI Foundry, AWS Bedrock AgentCore Runtime, Salesforce, SAP, and ServiceNow (150+ production deployments across the marketplace as of Q2 2026). Uses LangGraph for stateful workflow execution with checkpointing and human-in-the-loop gates — graph-based orchestration scales linearly where role-based (CrewAI) caps at ~5 agents and conversational (AutoGen) degrades past 3. Hook-based lifecycle management system (8 hook types: pre/post-execute, tool call, error, escalation, session lifecycle) enables cross-cutting concerns like audit logging and cost tracking without polluting agent code. Trajectory-level evaluation framework instruments every supervisor routing decision, tool selection, and reasoning step — measuring tool selection precision, reasoning chain coherence (Claude-as-judge with structured rubric), step-level latency, semantic similarity, hallucination, routing accuracy, and cost efficiency with A/B comparison and statistical significance testing. Reflection pattern available as opt-in: any agent can self-critique its output before responding, eliminating the last category of hallucinated outputs at the cost of one additional LLM call per task. OpenTelemetry GenAI semantic conventions (v1.37+) export every span — supervisor routing, cascade confidence, tool call, A2A cross-org call — into Datadog, Grafana, Elastic, or any OTLP-compatible stack with no vendor lock-in. Memory consolidation engine compacts session history into key facts, pattern insights, and archival summaries. Confidence-gated cascade routing starts with Haiku and escalates only when quality thresholds require it — reducing LLM spend 81% (up from 72%) through cascade optimization, Anthropic prompt caching with 73% hit rate (up from 37% after caching the supervisor system prompt and tool catalog), shared knowledge base, and intelligent model selection.",
    ...
    solution: "Unified orchestration with a supervisor agent classifying intent via Haiku (<200ms), routing to specialized agents, and enforcing five-layer deterministic guardrails (PII detection, budget enforcement, schema validation, compliance rules, content filtering). Three-tier memory architecture (Redis session state → PostgreSQL conversation history → pgvector long-term knowledge) eliminates redundant document processing. Per-agent circuit breakers prevent cascading failures. Confidence-gated cascade routing replaced static complexity classification — each agent's output is evaluated by a lightweight confidence classifier before deciding whether to accept or escalate to a more capable model, achieving an additional 20-30% cost reduction beyond the original 60% savings. Each agent is exposed as an A2A v1.2 cryptographically signed Agent Card with JSON-RPC task lifecycle (Linux Foundation Agentic AI Foundation governance), enabling external ecosystems (Salesforce, ServiceNow) to discover and invoke agents without custom integration. The pivot from pipeline to supervisor pattern cut average cost per task from $0.51 to $0.08, and cascade routing further reduced it to $0.05.",
    ...
    architecture: "Python/AsyncIO backend with LangGraph orchestration engine for stateful workflow execution, hook-based lifecycle management, and A2A v1.2 cryptographically signed Agent Card exposure via JSON-RPC for Google Cloud Marketplace listing (signing keys brokered through AgentGate's Vault integration). Pinecone vector database for shared knowledge retrieval across agents. PostgreSQL for conversation history with memory consolidation engine for session compaction. Redis Streams for inter-agent communication, session state, and cascade confidence scoring. Celery for distributed task execution. Claude Sonnet 4.6 / Opus 4.6 (with Anthropic prompt caching on the supervisor system prompt, tool catalog, and shared knowledge base — cache reads at 0.1× base, 73% hit rate) and GPT-4o via provider abstraction layer with confidence-gated cascade routing (lightweight classifier evaluates output quality before model escalation). Reflection pattern implemented as a LangGraph subgraph that runs the agent's output back through the same model with a structured critique prompt before final response. Trajectory-level eval framework runs Claude-as-judge against structured rubrics with Galileo-style trajectory_precision / trajectory_recall metrics, paired with outcome metrics. LangSmith for agent tracing, plus OpenTelemetry GenAI semantic conventions (v1.37+) exported via OTLP for any compatible observability stack.",
    pipeline: [
    ...
    metrics: [
      {value: "81%", label: "LLM Cost Reduction", prev: "$47K → $9K/month (prompt caching lift to 73% hit rate)"},
      {value: "94.2%", label: "Task Completion Rate", prev: "from 67%"},
      {value: "2.4s", label: "Avg Response Time", prev: "from 15.2s"},
      {value: "0", label: "Escaped Hallucinations", prev: "from 3/month (28-week window with Reflection pattern)"}
    ],
    tech: ["Python/AsyncIO","LangGraph","Claude Sonnet 4.6","Opus 4.6","Anthropic Prompt Caching","Reflection Pattern","GPT-4o","Pinecone","PostgreSQL","Redis Streams","Celery","LangSmith","OpenTelemetry GenAI","A2A Protocol v1.2","Linux Foundation Agentic AI","Google Cloud Agent Marketplace","Token-Rate Circuit Breakers","JSON-RPC"],
    ...
    operationalDocs: {
      slos: ["Agent Response P95 < 2s", "Orchestration Success Rate 99.0%", "Rate Limit Accuracy 100%", "Session Recovery < 10s", "Trajectory Precision > 0.85", "Reflection Pattern Latency Tax < 800ms", "Prompt Cache Hit Rate > 70%"],
      capacityHighlight: "Current: 12 agents, 5K tasks/day, 3 marketplace-listed. Scales to 100 agents at $1,600/mo (Redis Streams + LangGraph checkpoint optimization + prompt cache).",
      runbooks: ["Agent Deadlock / Infinite Loop", "LLM Provider Rate Limit Exceeded", "Supervisor Escalation Backlog", "Reflection Pattern Convergence Failure", "OTel Collector Export Lag"]
    },
    futureRoadmap: [
      {title: "Federated Agent Memory Across Organizations", description: "Allow agents from different organizations (running on the same Marketplace listing) to share pattern insights via privacy-preserving federated learning — improving routing accuracy and tool selection across customer deployments without exposing any tenant's raw data."},
      {title: "Agent-as-a-Judge Cross-Validation", description: "Implement multi-agent cross-validation where a separate evaluator agent reads execution traces and votes on success — replacing single-LLM-as-judge with quorum-based decisions for the highest-stakes workflows (claims denials, underwriting recommendations)."},
      {title: "Sub-Linear Cost Scaling via Cache Sharing", description: "Engineer prompt cache sharing across agents that consume the same shared knowledge base — turning N agents at 73% hit rate into N agents at >90% hit rate by treating the cache as a platform-level resource rather than per-agent state."}
    ]
  },
  {
    id: "clinical-ai",
    ...
    stage: "Production",
    timeline: "14 weeks",
    tagline: "HIPAA-Compliant AI for Prior Authorization & Revenue Cycle (Built for the CMS 7-Day PA Mandate)",
    description: "End-to-end clinical AI platform automating prior authorization, medical coding, and revenue cycle analytics for a 38-provider multi-specialty group — purpose-built for the CMS final rule that requires standard PA decisions within 7 calendar days and expedited within 72 hours, with electronic PA mandated for Medicare Advantage and Medicaid by 2026. Multi-EHR architecture (SMART on FHIR R4 + HL7v2) integrates Epic, Cerner, Athenahealth, and athenaOne in the same deployment — a deliberate moat against Epic Factory, which is Epic-only, while Meridian-style multi-specialty groups often run mixed EHR environments. A clinical NLP pipeline (SciSpaCy + MedCAT) extracts structured medical concepts from encounter notes, and three specialized AI agents handle PA generation, coding optimization, and denial prediction. Anthropic prompt caching applied to SMART on FHIR schema mappings, ICD-10/CPT reference materials, and payer-specific criteria libraries (BCBS NC, Aetna, UHC) — upgraded to the 1-hour extended cache TTL (payer policies update weekly to monthly, a clean fit), lifting hit rate from 68% to 86% and cutting per-encounter LLM cost from $0.13 to $0.08. Five-layer deterministic compliance guardrails ensure HIPAA adherence with field-level AES-256 encryption and immutable audit logging, plus full EU AI Act Annex III conformity for the August 2, 2026 high-risk enforcement date (clinical decision support is among the highest-risk Annex III categories): Article 8-15 documentation, conformity assessment workflows, EU database registration packages, and CE marking tracking auto-generated from the audit log for any provider network with EU operations.",
    ...
    architecture: "Python/Django backend with multi-EHR integration: SMART on FHIR R4 for Epic, Cerner, and Athenahealth, plus HL7v2 messaging for legacy system interop — vendor-neutral by design. SciSpaCy + MedCAT NLP pipeline for clinical concept extraction and SNOMED CT linking. Claude Enterprise Sonnet 4.6 (BAA-signed) powering three specialized agents with Anthropic prompt caching on FHIR schema, ICD-10/CPT references, and payer criteria libraries (cache reads at 0.1× base price, 68% hit rate). PostgreSQL with pgvector for knowledge retrieval. Redis for HIPAA-compliant session management (30-min auto-logoff). Celery for async clinical workflow processing. Field-level AES-256 encryption via Fernet. EU AI Act Annex III readiness package (technical documentation, human oversight, risk management) generated programmatically from the audit log for any provider network with EU operations.",
    pipeline: [
    ...
    metrics: [
      {value: "6.2h", label: "PA Generation Time", prev: "from 3.8-day manual cycle (well within CMS 7-day mandate)"},
      {value: "9.1%", label: "Denial Rate", prev: "from 18.4%"},
      {value: "$0.08", label: "Per-Encounter LLM Cost", prev: "from $0.13 (1-hour extended cache, 86% hit rate)"},
      {value: "$2.1M", label: "Est. Annual Revenue Impact", prev: "annualized from Q1"}
    ],
    tech: ["Python/Django","SMART on FHIR R4","Cerner","Athenahealth","SciSpaCy","MedCAT","Claude Sonnet 4.6","Anthropic Prompt Caching","PostgreSQL","pgvector","Redis","Celery","HL7v2","EU AI Act Annex III"],
    ...
    operationalDocs: {
      slos: ["Clinical Decision Latency P95 < 3s", "HIPAA Audit Completeness 100%", "PHI Encryption Coverage 100%", "Agent Accuracy > 95%", "PA Turnaround < 24hr (CMS 7-day mandate buffer)", "1-Hour Extended Cache Hit Rate > 80%", "EU AI Act Annex III Documentation Freshness < 7 days"],
      capacityHighlight: "Current: 500 encounters/day across Epic + Cerner + Athenahealth. Scales to 5,000 encounters/day at $2,800/mo (FHIR batch API + Redis session clustering + prompt cache).",
      runbooks: ["EHR FHIR Integration Failure (Epic / Cerner / Athenahealth)", "PHI Exposure Incident", "Clinical Agent Hallucination Detection", "CMS 7-Day Mandate SLA Breach", "Prompt Cache Invalidation on Payer Policy Update"]
    },
    futureRoadmap: [
      {title: "Federated Learning Across Provider Networks", description: "Pulled forward as the differentiating moat against Epic Factory: privacy-preserving federated model training across multiple provider groups so coding accuracy and denial prediction improve from shared patterns without exposing PHI — targeting 99%+ coding accuracy through cross-network intelligence by Q4 2026, available across Epic / Cerner / Athenahealth."},
      {title: "Auto-Updating Payer Rules Engine", description: "Build a payer criteria management system that auto-ingests coverage policy updates from BCBS, Aetna, and UHC via crawler + Claude extraction — eliminating the manual criteria maintenance that currently delays PA accuracy when payer policies change mid-quarter, with prompt cache invalidation tied to policy version hash."},
      ...
      {title: "On-Prem SLM Tier for Clinical Concept Extraction", description: "Add a Phi-4 14B (or Gemma 3 12B) on-prem inference tier below Claude for the high-volume clinical concept extraction path that currently runs through SciSpaCy + MedCAT plus cloud LLM. Eliminates PHI egress entirely for the 80% of routine extractions, builds a defensible privacy moat against Abridge and Nuance DAX (both cloud-only), and lands an additional 30-50% cost reduction beyond the 1-hour-cache wins."},
      {title: "Pharmacy-Benefit PA Expansion (CMS-0062-P Readiness)", description: "Extend the PA Engine from medical-benefit PA into pharmacy-benefit drug PA before the CMS-0062-P final rule effective date (October 1, 2027 for Medicare Advantage). The proposed rule mandates electronic PA for every drug requiring authorization across MA, Medicaid FFS, CHIP, and federally-facilitated QHPs — a clean two-quarter expansion that doubles addressable PA volume for existing customers."}
    ]
  }
];
...
export const FEATURED_CASES = [
  ...
  { id: "verified-marketplace", domain: "Marketplace", title: "Verified Services Marketplace", desc: "Two-sided marketplace with a five-layer trust stack. Projected $15M+ Year 1 GMV based on Q1 run rate, with 94% verification rate and <2% dispute rate.", stats: [{val:"$15M+",label:"Projected Y1 GMV"},{val:"<2%",label:"Dispute Rate"}] },
];
